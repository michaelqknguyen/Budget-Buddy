...
from budgetbuddy.paychecks.models import Paycheck
...
class TestPaycheckView(TestCase):
    ...
    def test_no_paychecks(self):
        ...
        response = response_attach_client(
            index(request), self.user.username, self.password
        )
    ...
    def test_redirect_first_paycheck(self):
        ...
        self.assertRedirects(
            response, reverse("paychecks:paycheck", kwargs={"paycheck_id": paycheck.id})
        )
    ...
    def test_one_paycheck(self):
        ...
        PaycheckFactory(user=user2)

        # proper response
        request_url = reverse(
            "paychecks:paycheck", kwargs={"paycheck_id": first_paycheck.id}
        )
    ...
    def test_404_wrong_paycheck(self):
        ...
        request_url = reverse(
            "paychecks:paycheck", kwargs={"paycheck_id": paycheck2.id}
        )
        response = self.client.get(request_url)
    ...
    def test_paystub_rendering(self):
        paycheck = PaycheckFactory(user=self.user)
        PaystubFactory(paycheck=paycheck, user=self.user)
    ...
    def test_deductions(self):
        ...
        deductions = DeductionFactory.create_batch(
            randint(1, 10), user=self.user, paycheck=paycheck
        )
    ...
    def test_update_paycheck(self):
        paycheck = PaycheckFactory(user=self.user)
        new_salary = Decimal(faker.Faker().random_int(min=1000, max=200000)).quantize(
            Decimal(".01")
        )
        data = {
            "company": paycheck.company,
...
class TestPaystubView(TestCase):
    ...
    def test_404_on_unauthorized_paycheck(self):
        ...
        request_url = reverse(
            "paychecks:paystub_add", kwargs={"paycheck_id": paycheck2.id}
        )
        response = self.client.get(request_url)
        self.assertEqual(response.status_code, 404)
    ...
    def test_add_paystub_forms(self):
        ...
        DeductionFactory.create_batch(
            randint(1, 5), user=self.user, paycheck=self.paycheck
        ...
        request_url = reverse(
            "paychecks:paystub_add", kwargs={"paycheck_id": self.paycheck.id}
        )
        response = self.client.get(request_url)

        paystub_form = response.context["paystub_form"]
        self.assertQuerySetEqual(
            paystub_form.fields["paycheck"].queryset, [self.paycheck]
        )
        ...
        for form in deposit_formset:
            self.assertQuerySetEqual(
                form.fields["money_account"].queryset, money_accounts, ordered=False
            )
    ...
    def test_paystub_delete(self):
        request_url = reverse(
            "paychecks:paystub_delete", kwargs={"pk": self.paystub.id}
        )
...
class TestDeductionView(TestCase):
    ...
    def test_update_deduction(self):
        request_url = reverse(
            "paychecks:deduction_edit", kwargs={"pk": self.deduction.id}
        )
        new_amount = faker.Faker().pydecimal(min_value=0, max_value=400, right_digits=2)
        data = {
            "paycheck": self.paycheck.id,
            "description": self.deduction.description,
            "deduction_type": self.deduction.deduction_type,
    ...
    def test_delete_deduction(self):
        request_url = reverse(
            "paychecks:deduction_delete", kwargs={"pk": self.deduction.id}
        )
        response = self.client.post(request_url, follow=True)
        self.assertRedirects(
            response,
    ...
    def test_error_deleting_other_user_deduction(self):
        ...
        paycheck = PaycheckFactory(user=user2)
        deduction = DeductionFactory(paycheck=paycheck, user=user2)
        request_url = reverse("paychecks:deduction_delete", kwargs={"pk": deduction.id})
