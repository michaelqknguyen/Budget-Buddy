...
from budgetbuddy.accounts.models import MoneyAccount, Transaction
from budgetbuddy.paychecks.models import Deduction, Paycheck, Paystub
...
class TransactionPaystubForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "amount_spent": forms.NumberInput(
                attrs={"class": "form-control contribution"}
            ),
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(*args, **kwargs)
        if self.user:
            self.fields["money_account"].queryset = MoneyAccount.objects.filter(  # type: ignore[attr-defined]
