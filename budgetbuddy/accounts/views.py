...
def index(request):
    ...
    money_accounts = (
        MoneyAccount.objects.filter(active__in=(True, money_active), user=user)
...
def account_view(request, account_id, account_type):
    ...
    money_accounts = MoneyAccount.objects.filter(active=True, user=user).order_by(
        "name"
    )
    budget_accounts = BudgetAccount.objects.filter(active=True, user=user).order_by(
        "name"
    )
    ...
    accounts = sorted(
        chain(money_accounts, budget_accounts), key=operator.attrgetter("name")
    )
    ...
    all_stock_shares = all_stock_shares.prefetch_related(
        "brokerage_account", "budget_account"
    )
    ...
    if account_id:
        ...
        balance = (
            all_transactions.filter(money_account__isnull=False)
            .aggregate(balance=Coalesce(Sum("amount_spent"), Decimal(0)))
            .get("balance", 0)
        )
    subset_transactions = all_transactions.filter(
    ...
    subset_spent = (
        subset_transactions.filter(paystub__isnull=True)
...
def transfer_transaction(request):
    if request.method == "POST":
        ...
        if from_account["money_or_budget"] != to_account["money_or_budget"]:
            messages.error(
                request,
                "Transfers can only be made between two budget accounts or two money accounts",
            )
            return return_url
        elif from_account["money_or_budget"] == "m":
            account_type = MoneyAccount
        elif from_account["money_or_budget"] == "b":
            account_type = BudgetAccount  # type: ignore[assignment]
        ...
        from_account_object = get_object_or_404(
            account_type, pk=from_account_id, user=request.user
        )
        to_account_object = get_object_or_404(
            account_type, pk=to_account_id, user=request.user
        )
        ...
        messages.success(
            ...
            "Transferred ${:.2f} from {} to {}".format(
                amount_transfer, from_account_object, to_account_object
            ),
        )
        return return_url
...
class TransactionUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_success_url(self):
        messages.success(
            self.request,
            "{} transaction successfully updated".format(self.object.description),
        )
...
class TransactionDeleteView(LoginRequiredMixin, DeleteView):
    ...
    def get_success_url(self):
        messages.success(
            self.request,
            "{} transaction successfully deleted".format(self.object.description),
        )
        if self.money_or_budget == "m":
            return reverse("budget:money_account", args=[self.object.money_account.id])
        elif self.money_or_budget == "b":
...
class BudgetAccountUpdateView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    def test_func(self):
        path = self.request.path
        account_id = re.search("b/(.*)/edit", path).group(1)  # type: ignore[union-attr]
...
class MoneyAccountUpdateView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    def test_func(self):
        path = self.request.path
        account_id = re.search("m/(.*)/edit", path).group(1)  # type: ignore[union-attr]
